...
export const ReceberDetailsModal: React.FC<ReceberDetailsModalProps> = ({
  ...
  const filteredOrders = useMemo(() => {
    return pendingOrders.filter(o => {
      ...
      if (subFilter === 'production' && o.status === 'entregue') return false;
      if (subFilter === 'delivered' && o.status !== 'entregue') return false;
  ...
  }, [pendingOrders, filterPeriod, subFilter, searchTerm, currentMonthStr, lastMonthStr]);
        ...
        <div className="relative w-full max-w-4xl max-h-[90vh] bg-zinc-950 border border-amber-500/30 rounded-3xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="p-4 bg-zinc-900/50 border-b border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-1.5 overflow-x-auto w-full sm:w-auto">
              <div className="flex items-center gap-1 bg-black/40 border border-white/10 rounded-2xl p-1 shrink-0">
                {[
                  { id: 'all', label: '📂 Todos' },
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-[11px] text-zinc-400">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <input 
                            ...
                            className="bg-black/60 border border-white/10 rounded-lg px-2 py-0.5 text-[10px] text-amber-400 outline-none focus:border-amber-500 font-bold"
                          />
                        </div>
                      </div>
                    </div>
